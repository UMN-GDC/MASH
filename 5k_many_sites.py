...
reps = 50
h2s = pd.DataFrame(np.zeros((2 * reps, 2)), columns = ["Combat", "SWD"]) 
h2s.index = pd.MultiIndex.from_product([["S50", "S100"], ["EQUAL"], list(range(reps))], names= ["Sites", "Distribution", "Rep"]) 
rng = np.random.default_rng(1234)
for sites, dist, rep in itertools.product([50, 100], ["EQUAL"], list(range(reps))) : 
...
h2s.to_csv("Simulate/AdjHE_sims/2k_many_sites.csv", index= True)
