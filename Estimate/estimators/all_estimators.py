...
def load_n_estimate(df, fixed_effects, nnpc, mp, GRM, PC_effect = "fixed", std=False, Method="AdjHE", random_groups=None, silent=False, homo=True, gcta="~/software/bin/gcta64"):
    ...
    if fixed_effects is None :
        ...
        fixed_effects = list(set(fixed_effects))
    ...
    if len(fixed_effects) != 0:
        # potential fix for Patsy not liking "." in variable names
        #fixed_effects = ["Q('" + ele + "')" for ele in fixed_effects]
    ...
    try :
        if Method == "AdjHE":
            ...
            result = GCTA(df, fixed_effects, nnpc, mp, GRM, gcta=gcta, method=Method, silent=False)
...
class h2Estimation():
    ...
    def estimate(self, PC_effect = "fixed"):
        ...
        if (args["fixed_effects"] == None) or (args["fixed_effects"] == []):
            fixed_combos = [[]]
        else:
            # Create the sets of covarates over which we can loop
            # This will return a list of lists of covariate names to regress on
            fixed_combos = [args["fixed_effects"][0:idx+1] for idx, c in enumerate(args["fixed_effects"])]
        ...
        if args["Method"] in ["Combat", "Covbat"] :
            ...
            if PC_effect in ["fixed", "mixed"] :
                FEs = ["pc_" + str(i + 1) for i in range(max(args["npc"]))] + args["fixed_effects"]
            else :
                FEs = args["fixed_effects"]

            no_missing = self.df[["FID", "IID"] + args["mpheno"] + FEs + [random_groups]].dropna()
            ...
            self.df[args["mpheno"]] = transformed_data
        ...
        for covs in tqdm(fixed_combos, desc = "Covariate sets"):
            ...
            for mp, nnpc in tqdm(itertools.product(self.mpheno, args["npc"]), desc = "Phenotype, PC combination counter"):
                ...
                if (not args["Naive"]) or (random_groups == None):
                    r = load_n_estimate(df=self.df, fixed_effects=covs, nnpc=nnpc,
                                        mp=mp, GRM=self.GRM, std=False, Method=args["Method"],
                    ...
                    for group in tqdm(groups, desc="# of subsets analyzed"):
                        try : 
                            ...
                            sub_result = load_n_estimate(df=sub_df, fixed_effects=[],  nnpc=nnpc, mp=mp, GRM=sub_GRM, std=False, Method=args["Method"], random_groups=None,
