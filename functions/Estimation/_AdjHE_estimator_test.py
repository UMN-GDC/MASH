...
class Precomputed() :
    def __init__(self, A, df, mp, npc):
        ...
        self.n = A.shape[0]
        self.trA = np.trace(A)
        self.trA2 = np.trace(np.matrix(A)**2)
        self.Sjs = None
        self.Tjs= None
        self.XXinv = None
        self.QSQ = None
    ...
    def compute_Sjs(self) :
        
        if self.npc ==0 :
            Precomputed["Sjs"] = 0
        else :
            # Grab the PCs
            PC_cols = [ col.startswith("pc")   for col in self.df ]
            # Select specified number of Pcs
            PC = np.array(self.df.iloc[:,PC_cols])[:, :self.npc]
            ...
            self.PPt = np.matmul(PCs, PCsT)
        
            # Calculated Sj's if not first time with this set of npcs
            self.Sjs= np.matmul(np.matmul(PCsT,self.A), PCs).flatten()
    
    def Compute_XXinv(self):
        n = self.A.shape[0]
    
        trA2 = np.trace(np.linalg.matrix_power(self.A, 2))
        trA = np.trace(self.A)
        
        topleft = trA2 - np.sum(self.Sjs** 2)
        offdiag= trA - np.sum(self.Sjs)
        bottomright = self.n- self.npc
            
        # Solve the regression problem
        self.XXinv = np.linalg.inv(np.matrix([[topleft, offdiag],
                                         [offdiag, bottomright]]))

        
    def compute_Tjs(self) :
        y = np.array(self.df[self.mp])
        # Calculate tj's
        if self.npc == 0:
            Tjs = 0
        else: 
            Tjs = np.matmul(np.matmul(y, self.PPt), y.T)
        self.Tjs = Tjs
        return Precomputed
...
def AdjHE_estimator(A, df, mp, Precomputed, npc=0, std=False):
    """
    ...
    Sjs = Precomputed["Sjs"]
    Tjs = Precomputed["Tjs"]
    XXinv=  Precomputed["XXinv"]
    trA2 = Precomputed["trA2"]
    trA = Precomputed["trA"]
    n = Precomputed["n"]
