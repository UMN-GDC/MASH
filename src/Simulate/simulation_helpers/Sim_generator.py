...
class pheno_simulator():
    def __init__(self, rng= None, nsubjects=1000, nSNPs=1000, plink_prefix=None, grmPrefix= None, covarFile=  None, subjAncestries = None):
        ...
        if plink_prefix is None :
            ...
            self.df = pd.DataFrame({"FID": np.arange(start=1, stop=nsubjects + 1).astype(str),
    ...
    def sim_pheno(self, h2Hom=0.5, h2Het=[0], alpha = -1, phenobasename = "Y", prop_causal = [0.1, 0.1], linearCombo = False,
                  siteEffects = False, riskGroups = False, interaction = False):
        self.h2Hom = h2Hom
        self.h2Het = h2Het
        self.alpha = alpha
        ...
        if linearCombo :
            # mean center all of the ^Y's  
            self.df.loc[:, self.df.columns.str.startswith('Y')] = self.df.filter(regex= '^Y').apply(lambda x: x - x.mean()) 
            # alternate 1 and -1 for the nphenos but without rounding to the nearest even number
            if interaction :
                betas = np.array([2* (-1)**i for i in range(self.nphenos)])
            else : 
                betas = np.array([(-1)**i for i in range(self.nphenos)])
            self.df["Z"] = np.dot(self.df.filter(regex='^Y').values, betas)
            # mean center Z
            self.df["Z"] = self.df["Z"] - self.df["Z"].mean()
